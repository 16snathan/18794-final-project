...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    ...
    dir_name = os.path.basename(
        # tf.compat.v1.gfile.Walk() returns sub-directory with trailing '/' when it is in
    ...
    for file_name in file_list:
      ...
      hash_name_hashed = hashlib.sha1(tf.compat.v1.compat.as_bytes(hash_name)).hexdigest()
...
def create_module_graph(module_spec):
  ...
  Returns:
    graph: the tf.compat.v1.Graph that was created.
  ...
  with tf.compat.v1.Graph().as_default() as graph:
    resized_input_tensor = tf.compat.v1.placeholder(tf.compat.v1.float32, [None, height, width, 3])
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
  ...
  # The tf.compat.v1.contrib.quantize functions rewrite the graph in place for
  # quantization. The imported model graph has already been rewritten, so upon
  # calling these rewrites, only the newly added final layer will be
  # transformed.
  if quantize_layer:
    if is_training:
      tf.compat.v1.contrib.quantize.create_training_graph()
    else:
      tf.compat.v1.contrib.quantize.create_eval_graph()
...
def add_jpeg_decoding(module_spec):
  ...
  input_height, input_width = hub.get_expected_image_size(module_spec)
  input_depth = hub.get_num_image_channels(module_spec)
  jpeg_data = tf.compat.v1.placeholder(tf.compat.v1.string, name='DecodeJPGInput')
  decoded_image = tf.compat.v1.image.decode_jpeg(jpeg_data, channels=input_depth)
  # Convert from full range of uint8 to range [0,1] of float32.
  decoded_image_as_float = tf.compat.v1.image.convert_image_dtype(decoded_image,
                                                        tf.compat.v1.float32)
  ...
  resize_shape_as_int = tf.compat.v1.cast(resize_shape, dtype=tf.compat.v1.int32)
